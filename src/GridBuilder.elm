module GridBuilder (GridPosition, isInRange, isTaken, getPosition, getPositions) where
...
isTaken : List ( GridPosition, { a | height : Int, width : Int } )
        -> GridPosition
        -> Bool
...
getPosition perRow grid item =
  let
    last = List.reverse grid |> List.head

    nextPosition position =
      if position.x + item.width < perRow
      then { x = position.x + 1, y = position.y }
      else { x = 0, y = position.y + 1 }

    firstTryPosition =
      case last of
        Just (lastPosition, lastItem) ->
          nextPosition { x = lastPosition.x + lastItem.width - 1
                       , y = lastPosition.y }
        Nothing -> { x = 0, y = 0 }

    tryPosition position =
      case isTaken grid position of
        True -> tryPosition (nextPosition position)
        False -> position
